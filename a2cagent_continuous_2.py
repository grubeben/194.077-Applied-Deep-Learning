...
class A2CAgent(keras.Model):
    ...
    def __init__(self,s0, act_space_high, act_space_low, state_space_samples, obs_dim, state_normalization, batch_normalization, use_existing_policy, add_branch_layer):
        ...
        ### common layers
        self.my_weight_dict["dense1"] = layers.Dense(self.nodes_per_dense_layer, activation=tfa.activations.mish, kernel_initializer=keras.initializers.he_normal())
        self.my_weight_dict["batch_norm1"]=layers.BatchNormalization()
        self.my_weight_dict["dense2"] = layers.Dense(self.nodes_per_dense_layer, activation=tfa.activations.mish, kernel_initializer=keras.initializers.he_normal())
        self.my_weight_dict["batch_norm2"]=layers.BatchNormalization()
        ### critic layers
        self.my_weight_dict["value_inbetween1"] = layers.Dense(self.nodes_per_dense_layer/2, activation=tfa.activations.mish, kernel_initializer=keras.initializers.he_normal())
        self.my_weight_dict["batch_norm_value"]=layers.BatchNormalization()
        self.my_weight_dict["value"] = layers.Dense(1) #outlet 1
        ### actor layers
        self.my_weight_dict["norm_inbetween1"] = layers.Dense(self.nodes_per_dense_layer/2, activation=tfa.activations.mish, kernel_initializer=keras.initializers.he_normal())
